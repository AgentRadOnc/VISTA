...
class VISTA3D2(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        return logits    def __init__(self, image_encoder, class_head, point_head, text_head, feature_size):
...
class VISTA3DText(VISTA3D2):
    def __init__(self, image_encoder, class_head, text_head, feature_size):
        """Initialize the VISTA3DText model specialized for text-based segmentation.
        
        Args:
            image_encoder: Encoder network for processing input images
            class_head: Network head for class-based segmentation (used for auto layers)
            text_head: Network head for text-based segmentation
            feature_size: Size of feature vectors produced by the encoder
        """
        # Initialize with a dummy point_head since we won't use it
        dummy_point_head = nn.Identity()
        super().__init__(image_encoder, class_head, dummy_point_head, text_head, feature_size)
    ...
    def forward(
        ...
        **kwargs,
        ...
        """Forward pass for VISTA3DText model using only text embeddings.
        
        This method specializes in text-based segmentation using the auto layers
        from the base VISTA3D2 model.
        
        Args:
            input_images: Tensor [1, 1, H, W, D] - Input 3D image
            text_embeddings: Tensor - Text prompt embeddings for text-based segmentation
            patch_coords: Optional slice object - Coordinates of current patch in sliding window inference
            **kwargs: Additional keyword arguments
                - keep_cache: bool - Whether to cache image embeddings for future use
        ...
        if (
            ...
            # Use with_label=True to get out_auto for the text head
            out, out_auto = self.image_encoder(
                input_images,
                with_point=False,
                with_label=True,
        ...
        # Use the text_head with out_auto instead of out for better performance
        logits = self.text_head(out_auto, text_embeddings=text_embeddings)
        
        if kwargs.get("keep_cache", False):
            self.image_embeddings = out.detach()
...
class VISTA3DText(VISTA3D2):
    def __init__(self, image_encoder, text_head, feature_size):
        """Initialize the VISTA3DText model specialized for text-based segmentation.
        ...
        # Initialize with a dummy point_head since we won't use it
        dummy_point_head = nn.Identity()
        dummy_class_head = nn.Identity()  # Dummy class head, not used in this model
        super().__init__(image_encoder, dummy_class_head, dummy_point_head, text_head, feature_size)
